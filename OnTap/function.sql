...
-- 7.11
-- b. Hãy xây dựng hàm thống kê số lượng nhân viên mỗi phòng với tên phòng nhập từ
-- bàn phím
create function cau1b(@tenphong nvarchar(30))
returns int
as 
begin
	declare @soluong int
...
select dbo.cau1b(N'Kế toán')

-- c. Hãy viết hàm thống kê xem tên sản phẩm x đã xuất được bao nhiêu sản phẩm trong
-- ngày y, với x,y nhập từ bản phím.
create function cau1c(@tensp nchar(20), @ngay int)
returns int
as
begin
	declare @so int
	set @so = (select sum(soluongx) 
				from xuat x
				inner join SanPham sp on x.MaSP = sp.MaSP
				inner join PXuat px on x.SoHDX = px.SoHDX
				where TenSP = @tensp and day(NgayXuat) = @ngay)
	return @so
end

select dbo.cau1c('Vjoy3', 17)

-- d. Hãy viết hàm trả về số diện thoại của nhân viên đã xuất số hóa đơn x, với x nhập từ
-- bàn phím
create function cau1d(@hdx nchar(10))
returns nvarchar(20)
as
begin
...
select dbo.cau1d('X04')

-- e. Hãy viết hàm thống kê tổng số lượng thay đổi nhập xuất của tên sản phẩm x trong
-- năm y, với x,y nhập từ bàn phím.

create function cau1e(@x nvarchar(30), @y int)
returns int
as 
begin
	declare @dem int
	set @dem = (select sum(soluongn)-sum(soluongx)
				from SanPham sp
...
select dbo.cau1e('Vjoy3', 2022)

-- f. Hãy viết hàm thống kê tổng số lượng sản phầm của hãng x, với tên hãng nhập từ
-- bàn phím
create function cau1f(@tenhang nvarchar(20))
returns int
as
begin 
	declare @dem int
	set @dem = (select sum(soluong)
				from SanPham sp
...
select dbo.cau1f('OPPO')


-- 7.22
-- Hãy tạo hàm Đưa ra danh sách các hãng sản xuất có địa chỉ nhập vào từ bàn phím
-- (Lưu ý – Dùng hàm Like để lọc).
create function cau2a(@dc nvarchar(30))
returns @bang table (
	MaHangSX nchar(10),
	TenHang nvarchar(20),
	DiaChi nvarchar(30),
...
begin
insert into @bang
select MaHangSX, TenHang, DiaChi, SoDienThoai, Email
from HangSX
where DiaChi = @dc
return 
end

select *from cau2a('Korea')
